...
impl StreamFragmenter {
    ...
    fn infer_internal_table_catalog(
        ...
        for column_desc in &arrangement_info.column_descs {
            internal_table_catalog_builder.add_column_desc_from_field_without_order_type(
                &Field::from(&ColumnDesc::from(column_desc)),
            );
        }
        ...
        let a = internal_table_catalog_builder.build(distribution_key, append_only);
        println!("rewrite, delta join table columns: {:?}", a.columns);
        println!("rewrite, delta join table pks: {:?}", a.pk);
        a
    }
}
